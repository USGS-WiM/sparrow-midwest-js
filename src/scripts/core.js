...
require([
  ...
  app.executeIdentifyTask = function (evt) {
    ...
    for (i in visLayers) {
      ...
      if (
        visLayers[i].id === "streamflowCalibration" &&
        app.map.getLayer("streamflowCalibration").visible === true
      ) {
        calibrationId = app.map.getLayer("streamflowCalibration")
          .visibleLayers[0];
        app.identifyParams.layerIds.push(calibrationId);
      }
      if (
        visLayers[i].id === "sedimentCalibration" &&
        app.map.getLayer("sedimentCalibration").visible === true
      ) {
        calibrationId = app.map.getLayer("sedimentCalibration")
          .visibleLayers[0];
        app.identifyParams.layerIds.push(calibrationId);
      }
    }
          ...
          if (response.length >= 1) {
            $.each(response, function (index, responseObj) {
              ...
              if (responseObj.layerId === 36) {
                ...
                calibrationTemplate.setContent(
                  "<div><b>Station Name:</b> " +
                    ...
                    chartUnits +
                    ": </b>" +
                    Math.round(responseObj.feature.attributes.PLOAD_TOTA) +
                    "</div><br>"
                );
              ...
              if (responseObj.layerId === 38) {
                ...
                var calibrationTemplateN = new esri.InfoTemplate();
                calibrationTemplateN.setTitle(
                  "SPARROW " + model_q + " Calibration Site"
                );
                //UPDATE important! make sure the field names below match what is in the REST layer
                calibrationTemplateN.setContent(
                  "<div><b>Station Name:</b> " +
  ...
  app.getLayerConfigObject = function (sparrowLayerId) {
    var configObject = (function (tempLayerId) {
      switch (tempLayerId) {
        ...
        /////BEGIN STREAMFLOW LAYERS___________________________________________________________
        case 18:
          return Catchments_q; // Nitro catchments
        case 19:
          return Group3_q; // HUC8
        case 20:
          return Group2_q; // Trib
        case 21:
          return Group1_q; // Main River Bains
        case 22:
          return ST_q; // State
        case 23:
          return Catchments_st_q; // cats w/ state divisions
        case 24:
          return Group3_st_q; // grp3 w/ state divisions
        case 25:
          return Group2_st_q; // grp2 w/ state divisions
        case 26:
          return Group1_st_q; // grp1 w/ state divisions
        /////END STREAMFLOW LAYERS___________________________________________________________
        /////BEGIN SUSPENDED SEDIMENT LAYERS___________________________________________________________
        case 27:
          return Catchments_ss; // Nitro catchments
        case 28:
          return Group3_ss; // HUC8
        case 29:
          return Group2_ss; // Trib
        case 30:
          return Group1_ss; // Main River Bains
        case 31:
          return ST_ss; // State
        case 32:
          return Catchments_st_ss; // cats w/ state divisions
        case 33:
          return Group3_st_ss; // grp3 w/ state divisions
        case 34:
          return Group2_st_ss; // grp2 w/ state divisions
        case 35:
          return Group1_st_ss; // grp1 w/ state divisions
        /////END SUSPENDED SEDIMENT LAYERS___________________________________________________________
      }
    })(sparrowLayerId);
  ...
  function showChart(response) {
    ...
    if ($('.radio input[type="radio"]:checked')[0].id == "radio2") {
      colorArr = nitroColors;
    }
    if ($('.radio input[type="radio"]:checked')[0].id == "radio3") {
      colorArr = streamflowColors;
    }
    if ($('.radio input[type="radio"]:checked')[0].id == "radio4") {
      colorArr = sedimentColors;
    }
    ...
    $(function () {
        ...
        chart: {
          ...
          events: {
            load: function () {
              ...
              chart.renderer
                .text(
                  '<div><i class="fa fa-mouse-pointer"></i> Click on legend elements to toggle on/off sources</div>',
                  100,
                  40,
                  true
                )
      ...
      if (app.chosenSource) {
        $.each(
          $("#chartWindowContainer").highcharts().series,
          function (i, ser) {
